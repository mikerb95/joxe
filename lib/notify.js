...
async function sendNtfy({ ntfyTitle, title, body, tags = "bell", stylist = null, priority = null }) {
  ...
  await Promise.allSettled([...topics].map(topic =>
    fetch(`https://ntfy.sh/${encodeURIComponent(topic)}`, {
      method: "POST",
      headers: {
        Title: ntfyTitle ?? title,
        Tags: tags,
        // ntfy usa la prioridad para saltarse el modo silencio del teléfono.
        // Solo se manda cuando el aviso lo amerita, para que no pierda efecto.
