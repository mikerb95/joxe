...
// Convierte una fila del backup al texto que va en la columna `value`.
// Es la inversa exacta de kvDump. La rama del string sin marca existe para los
// backups v1, que guardaban el valor crudo sin flag.
function restoreValue(row) {
  if (row.raw === true) return String(row.value);
  if (typeof row.value === "string") {
    try { JSON.parse(row.value); } catch { return row.value; }
  }
