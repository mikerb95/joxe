...
export async function kvDump() {
  ...
  return res.rows.map(r => {
    // El valor se guarda como texto JSON. Se parsea para que el backup sea
    // legible y diffeable. Si no es JSON válido se conserva el texto crudo y se
    // marca con raw:true: al restaurar hay que escribirlo sin volver a
    // serializarlo, porque un texto plano y un JSON string no se distinguirían.
    try {
      return { key: r.key, value: JSON.parse(r.value), updated_at: Number(r.updated_at) };
    } catch {
      return { key: r.key, value: r.value, raw: true, updated_at: Number(r.updated_at) };
    }
  });
}

// Convierte una fila del backup al texto que va en la columna `value`: la
// inversa exacta de kvDump. Solo el flag raw decide, nunca el tipo del valor;
// si no, un string guardado por kvSet ("hola") volvería a la base sin comillas
// y dejaría de ser JSON legible. Los backups v1 no traen el flag: sus valores
// vuelven serializados, que es como los escribió kvSet en su momento.
function restoreValue(row) {
  return row.raw === true ? String(row.value) : JSON.stringify(row.value);
}
...
export async function kvRestore(rows, { replace = false } = {}) {
  ...
  for (const r of rows) {
    ...
    writes.push({
      sql: `INSERT INTO kv (key, value, updated_at) VALUES (?, ?, ?)
            ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`,
      args: [r.key, restoreValue(r), updatedAt],
    });
  }

  // Un "replace" sin filas válidas vaciaría la base entera. Eso nunca es una
  // restauración: es un archivo vacío o corrupto. Se corta antes de borrar nada.
  if (replace && !writes.length) {
    throw new Error("El respaldo no tiene registros válidos: no se borró nada.");
  }
  if (!writes.length) return 0;

  // batch() runs all statements in a single implicit transaction (all-or-nothing)
  await getDb().batch(replace ? ["DELETE FROM kv", ...writes] : writes, "write");

  // El cache en memoria quedó apuntando a los datos viejos.
  kvInvalidate();
...
// ------------------------------------------------------------
// Celulares: se guardan en E.164 ("+573001234567"). El formulario manda el
// indicativo que eligió el cliente. Las citas de antes (y una página vieja
// en caché) traen 10 dígitos sin indicativo: eran todos celulares
// colombianos, así que se leen como +57.
// La misma regla está en el front (telefono.jsx). Si cambia aquí, cámbiala
// también allá.
// ------------------------------------------------------------
...
// "" si sirve. Colombia pide 10 dígitos; para otros países no hay una regla
// única, solo el rango de E.164. El front es más fino (conoce cada
// indicativo), así que aquí nunca se rechaza algo que el formulario aceptó.
export function phoneError(v) {
  const e = normPhone(v);
  if (!e.startsWith("+")) return "Escribe un celular válido.";
  if (e.startsWith("+57")) return e.length === 13 ? "" : "El celular debe tener 10 dígitos.";
  const n = e.length - 1;
  return n >= 7 && n <= 15 ? "" : "Escribe un celular válido.";
}
