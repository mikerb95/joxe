...
const DEFAULT_SERVICES = [
  ...
  { id:"s6",   name:"Keratina alisado permanente dama",                    price:100000, dur:240, note:"desde" },
  { id:"s14",  name:"Keratina alisado permanente hombre (+ corte gratis)", price:90000, dur:240 },
  { id:"s15",  name:"Ondulado permanente hombre (+ corte gratis)",         price:130000, dur:120 },
  { id:"s16",  name:"Depilación de cejas con cera",                        price:10000, dur:60 },
  { id:"s17",  name:"Depilación de cejas con cuchilla",                    price:5000,  dur:60 },
  { id:"s18",  name:"Limpieza facial",                                     price:45000, dur:60 },
  { id:"s19",  name:"Corte + limpieza facial",                             price:55000, dur:60 },
  { id:"s7",   name:"Asesoría de imagen",                                  price:0,     dur:60 },
];
...
const BookingPortal = () => {
  ...
  return (
    <PortalShell tone="ivory" header={
      ...
      <main style={{
        flex: 1, padding: "56px 40px", paddingBottom: 120, maxWidth: 900,
...
// ——————————————————————————————————————————————
// WHATSAPP BLOB
// Se aparta solo cuando su zona choca con un CTA marcado [data-wa-avoid],
// para no tapar botones (típico en mobile con "Continuar").
// ——————————————————————————————————————————————
const useWAAvoid = (offset, size) => {
  const [away, setAway] = React.useState(false);
  React.useEffect(() => {
    let frame = null;
    const check = () => {
      frame = null;
      const right = window.innerWidth - offset;
      const bottom = window.innerHeight - offset;
      const zone = { left: right - size, right, top: bottom - size, bottom };
      const pad = 16;
      const hit = Array.from(document.querySelectorAll("[data-wa-avoid]")).some(el => {
        const r = el.getBoundingClientRect();
        if (!r.width || !r.height) return false;
        return r.left < zone.right + pad && r.right > zone.left - pad &&
               r.top < zone.bottom + pad && r.bottom > zone.top - pad;
      });
      setAway(hit);
    };
    const schedule = () => { if (frame == null) frame = requestAnimationFrame(check); };
    schedule();
...
const WABlob = () => {
  ...
  const size = mobile ? 48 : 52;
  const away = useWAAvoid(offset, size);
  ...
  return (
    <a
      ...
      style={{
        ...
        right: offset, zIndex: 9999,
        display: "flex", alignItems: "center", gap: 10,
        background: "#25D366", color: "#fff",
        borderRadius: 999, textDecoration: "none",
        padding: mobile ? 12 : (hovered ? "14px 22px 14px 18px" : "14px"),
        boxShadow: "0 4px 24px rgba(37,211,102,0.35)",
        transition: "opacity 0.2s, transform 0.25s cubic-bezier(.4,0,.2,1), padding 0.25s cubic-bezier(.4,0,.2,1)",
        overflow: "hidden", whiteSpace: "nowrap",
        opacity: away ? 0 : 1,
        transform: away ? "translateY(160%)" : "none",
        pointerEvents: away ? "none" : "auto",
      }}
    >
      <svg width={mobile ? 22 : 24} height={mobile ? 22 : 24} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style={{ flexShrink: 0 }}>
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
      </svg>
      {hovered && !mobile && (
